<x-layouts.app title="الموردون" active="catalog.suppliers">
    ...
    <div class="mb-4 flex flex-wrap items-center gap-3">
        <form method="GET" class="relative flex-1 min-w-48">
            ...
            <input type="search" name="search" value="{{ request('search') }}" placeholder="ابحث بالاسم"
        ...
        @can('suppliers.create')
            <a href="{{ route('suppliers.create') }}"
               class="flex shrink-0 items-center gap-1.5 rounded-lg bg-brand-600 px-3.5 py-2 text-sm font-semibold text-white hover:bg-brand-700">
                <x-icon name="plus" class="size-4" /> مورد جديد
            </a>
        @endcan
    </div>
            ...
            <table class="w-full text-sm">
                <thead class="bg-slate-50 text-[11px] uppercase text-slate-500">
                    <tr>
                        ...
                        <th class="px-4 py-2.5 text-start font-semibold">مسؤول التواصل</th>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($suppliers as $supplier)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="nums px-4 py-3 text-slate-600">{{ $supplier->phone ?? '—' }}</td>
                            <td class="px-4 py-3 text-slate-600">{{ $supplier->email ?? '—' }}</td>
                            <td class="px-4 py-3 text-center">
                                <span @class([
                                    'rounded px-2 py-1 text-[11px] font-medium',
                                    'bg-emerald-100 text-emerald-700' => $supplier->status->value === 'active',
                                    'bg-slate-100 text-slate-600' => $supplier->status->value !== 'active',
                                ])>{{ $supplier->status->label() }}</span>
                            </td>
                            <td class="px-4 py-3">
                                <div class="flex items-center justify-center gap-2">
                                    @can('suppliers.update')
                                        <a href="{{ route('suppliers.edit', $supplier) }}" class="text-slate-400 hover:text-brand-600" title="تعديل">
                    ...
                    @empty
                        <tr><td colspan="6" class="px-4 py-10 text-center text-slate-400">لا يوجد موردون بعد</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
