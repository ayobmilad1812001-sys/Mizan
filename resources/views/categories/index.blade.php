<x-layouts.app title="التصنيفات" active="catalog.categories">
    ...
    <div class="grid grid-cols-1 gap-5 lg:grid-cols-3">
        <div class="rounded-xl border border-slate-200 bg-white p-5 shadow-sm lg:col-span-1">
            <h2 class="mb-4 text-sm font-bold text-slate-900">إضافة تصنيف</h2>
            <form method="POST" action="{{ route('categories.store') }}" class="space-y-3">
                ...
                <div>
                    ...
                    <textarea name="description" rows="2"
                              class="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/20">{{ old('description') }}</textarea>
                </div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @forelse ($categories as $category)
                        <tr class="hover:bg-slate-50">
                            ...
                            <td class="nums px-4 py-3 text-center text-slate-600">{{ $category->products_count }}</td>
                            <td class="px-4 py-3 text-center">
                                <span @class([
                                    'rounded px-2 py-1 text-[11px] font-medium',
                                    'bg-emerald-100 text-emerald-700' => $category->status->value === 'active',
                                    'bg-slate-100 text-slate-600' => $category->status->value !== 'active',
                    ...
                    @empty
                        <tr><td colspan="4" class="px-4 py-8 text-center text-slate-400">لا توجد تصنيفات بعد</td></tr>
                    @endforelse
                </tbody>
            </table>
